...
class LangChainQuantaBot:
    ...
    def setup_ensemble_retriever(self, documents: List[Document]):
        """Hybrid semantic + BM25 ensemble, wrapped in cross-encoder reranking."""
        ...
        # Cross-encoder rerank on top of the ensemble.
        # Override with QUANTA_RERANKER_MODEL env var; default is BGE-reranker-v2-m3.
        from langchain_community.cross_encoders import HuggingFaceCrossEncoder
        reranker_name = os.getenv("QUANTA_RERANKER_MODEL", "BAAI/bge-reranker-v2-m3")
        cross_encoder_model = HuggingFaceCrossEncoder(model_name=reranker_name)
        compressor = CrossEncoderReranker(model=cross_encoder_model, top_n=12)
    ...
    def preprocess_documents(self, file_paths: List[str], batch_size: int = 10, file_mapping: dict = None):
        ...
        if len(all_documents) > 20:  # Use ensemble for larger document sets
            ...
            self._add_to_parent_retriever_in_batches(parent_retriever, all_documents)
    ...
    @staticmethod
    def _add_to_parent_retriever_in_batches(parent_retriever, documents, input_batch: int = 1500):
        # Each input doc fans out to ~2-3 child chunks; keep child count per add_documents
        # call under Chroma's ~5461 hard limit.
        for start in range(0, len(documents), input_batch):
            parent_retriever.add_documents(documents[start:start + input_batch])
    ...
    def query_stream(self, question: str):
        ...
        Yields answer text incrementally. After iteration completes, the
        full answer and retrieved source documents are available on
        ``self.last_answer`` and ``self.last_sources``.
        """
        if self.retrieval_chain is None:
            raise ValueError("Please preprocess documents first using preprocess_documents()")

        chat_history = self.memory.chat_memory.messages[-10:]
        enhanced_input = {"input": question, "chat_history": chat_history}

        self.last_answer = ""
        self.last_sources = []

        for chunk in self.retrieval_chain.stream(enhanced_input):
            if "answer" in chunk and chunk["answer"]:
                self.last_answer += chunk["answer"]
                yield chunk["answer"]
            if "context" in chunk and chunk["context"]:
                self.last_sources = chunk["context"]

        self.memory.save_context(
            {"input": question},
            {"output": self.last_answer},
        )
